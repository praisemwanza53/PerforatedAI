...
def safe_roc_auc(y_true: np.ndarray, y_score: np.ndarray) -> float:
    """
    roc_auc_score fails if only one class present in y_true.
...
def compute_auc_multitask(
    ...
    """
    Multi-task AUC: average over tasks with valid labels.
    - MoleculeNet labels often include missing labels; we mask out NaNs.
    """
    ...
    for t in range(T):
        ...
        yt = y_true[m, t].numpy()
        yp = y_prob[m, t].numpy()
        # Some datasets store labels as {0,1} floats; keep as is
...
def main():
    ...
    ap.add_argument("--data_frac", type=float, default=1.0,
                    help="Use fraction of training data to demonstrate data efficiency.")
    args = ap.parse_args()
    ...
    # Simple split (consistent + easy to reproduce)
    n = len(dataset)
    n_train = int(0.8 * n)
    n_val = int(0.1 * n)
    ...
    # Data fraction for training only
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        test_loss, test_auc = eval_epoch(model, test_loader, device)
        ...
        print(
            ...
            f"lr={lr_now:.2e} params={params} time={dt:.1f}s data_frac={args.data_frac}"
        )
